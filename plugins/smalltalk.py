...
class smalltalk(Plugin):
    ...
    @register("en-US", ".*I.*am.*sorry.*")
    def st_sorry(self, speech, language):
        if language == 'en-US':
            self.say("It is ok.")
    ...
    @register("en-US", ".*are.*you.*jailbroken.*")
    def st_jb(self, speech, language):
        if language == 'en-US':
            self.say("How else are you talking to me.")
        self.complete_request()
        
    @register("en-US", "(.*tell.*)|(.*message.*)|(.*imessage.*)|(.*text.*)")
    def st_tell(self, speech, language):
        if language == 'en-US':
            self.say("I can not message yet.")
            self.say("I might be able to in the future.")
        self.complete_request()
        
    @register("en-US", ".*why.*do.*you.*not.*")
    def st_wdyu(self, speech, language):
        if language == 'en-US';
            self.say("I am not perfect.")
            self.say("I was remade by a 15 year old developer named Tristen Russ AKA Playfrog4u.")
            self.say("If you want to make a request to add commands to siri. Please iMessage him at admin@playfrog4u.com")
            self.say("Thank You!")
        self.complete_request()
